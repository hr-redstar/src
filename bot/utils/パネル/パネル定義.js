...
function adminEmbed(guild, cfg) {
  ...
  const driverPanelLink = dp.channelId && dp.messageId
  ...
  return new EmbedBuilder()
    ...
    .addFields(
      ...
      { name: "運営者用ログスレッド", value: cfg.adminLog.threadId ? `<#${cfg.adminLog.threadId}>` : "未作成", inline: false },
...
function driverPanelEmbed(driverCount = 0) {
  return new EmbedBuilder()
    ...
    .addFields(
      { name: "送迎者数", value: `${driverCount}人（今から行けます を押している送迎者人数）`, inline: false }
    );
}
