...
async function findUserMemoChannel({ guild, userId, categoryId, role = 'driver' }) {
  ...
  if (foundChannels.length === 0) {
    const byOldTopic = guild.channels.cache.filter(
      (ch) => ch.type === 0 && ch.parentId === categoryId && ch.topic === `user-memo:${userId}`
    );
  ...
  if (foundChannels.length === 0) {
    const channels = guild.channels.cache.filter(
      (channel) => channel.parentId === categoryId && channel.isTextBased()
    );
    ...
    for (const channel of channels.values()) {
      try {
        ...
        const found = messages.find(
          (msg) => msg.author.bot && msg.content.includes(`ユーザーID：${userId}`)
        );
        if (found) {
...
async function archiveOldMemoChannel(channel, userId) {
  const newName = channel.name.includes('｜old') ? channel.name : `${channel.name}｜old`;
 ...
 */
async function notifyDuplicateChannels({
  guild,
  userId,
  role,
  foundChannels,
