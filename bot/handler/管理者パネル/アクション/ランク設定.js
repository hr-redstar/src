const { ActionRowBuilder, UserSelectMenuBuilder, StringSelectMenuBuilder, StringSelectMenuOptionBuilder, EmbedBuilder } = require('discord.js');
    ...
    async execute(interaction) {
        return interactionTemplate(interaction, {
            ...
            async run(interaction) {
                ...
                await interaction.editReply({ content: 'ランクを設定するユーザーを選択してください。', components: [row] });
            }
        });
    }
};
...
module.exports.handleUserSelect = async function (interaction) {
    return interactionTemplate(interaction, {
        ...
        async run(interaction) {
            ...
            if (ranks.length === 0) {
                return interaction.editReply({ content: '❌ ランク階級が登録されていません。先に「ランク階級登録」を行ってください。', components: [] });
            }
