const { UserSelectMenuBuilder, ActionRowBuilder, EmbedBuilder, ButtonBuilder, ButtonStyle } = require('discord.js');
...
module.exports = {
    ...
    async startFlow(interaction) {
        ...
        return interaction.reply({ content: '👤 評価を確認したいユーザーを選択してください。', components: [row], flags: 64 });
    },
    ...
    async showStats(interaction) {
        return autoInteractionTemplate(interaction, {
            ...
            async run(interaction) {
                ...
                const targetUser = await interaction.guild.members.fetch(targetUserId).catch(() => ({ displayName: targetUserId, user: { displayAvatarURL: () => null } }));
                ...
                const embed = new EmbedBuilder()
                    ...
                    .addFields(
                        { name: '総合評価', value: `**${stats.average}** ${starLine} (${stats.totalCount}件)`, inline: false },
                        {
                            name: '評価内訳',
                            value: [
    ...
    async showComments(interaction, targetUserId) {
        return autoInteractionTemplate(interaction, {
            ...
            async run(interaction) {
                ...
                const targetUser = await interaction.guild.members.fetch(targetUserId).catch(() => ({ displayName: targetUserId }));
