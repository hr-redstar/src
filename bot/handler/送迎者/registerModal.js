...
module.exports = async (interaction) => {
    ...
    const capacity = Number(
        interaction.fields.getTextInputValue("driver:input:capacity")
    );
    ...
    if (config.categories?.userMemo) {
        ...
        if (memoChannel) {
            ...
            const messageId = await getRegistrationMessageId(interaction.guild.id, interaction.user.id, 'driver');
            ...
            if (messageId) {
                ...
                // メッセージIDを保存
                if (sentMessage) {
                    await saveRegistrationMessageId(interaction.guild.id, interaction.user.id, sentMessage.id, 'driver').catch(err => {
