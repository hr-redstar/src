        ...
        if (jsonFiles.length > 0) {
            ...
            if (drivers.length > 0) {
                const lines = drivers.map((d, i) => {
                    const time = new Date(d.timestamp).toLocaleTimeString('ja-JP', { hour: '2-digit', minute: '2-digit' });
        ...
        if (filteredFiles.length > 0) {
            ...
            for (const fileKey of filteredFiles) {
                ...
                if (data) {
                    ...
                    lines.push(`【${status}】<@${data.driverId}> ➔ <@${data.passengerId}> (${data.direction})${carpoolText}`);
                }
            }
