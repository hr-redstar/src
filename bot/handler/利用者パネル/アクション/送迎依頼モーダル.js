        ...
        async run(interaction) {
            ...
            if (!driverData) {
                await interaction.editReply({
                    content: "⚠️ 申し訳ありません、現在待機中の送迎車がいません。\nしばらく待ってから再度お試しください。"
                });
                return;
            }
                        ...
                        try {
                            ...
                            if (memoCategoryId) {
                                ...
                                if (memoChannel) {
                                    ...
                                    const thread = await memoChannel.threads.create({
            ...
            // 4. 個人メッセージ送信 (Embed)
            const vcLink = vcChannel ? `[プライベートVCはこちら](https://discord.com/channels/${guildId}/${vcChannel.id})` : "VC作成失敗";
